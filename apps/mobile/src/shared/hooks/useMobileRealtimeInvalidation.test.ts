...
describe("getMobileRealtimeInvalidationKeys", () => {
  ...
  it("refreshes the profile-change-requests queue when a request row changes", () => {
    expect(
      getMobileRealtimeInvalidationKeys("token-1", "profile_change_requests"),
    ).toEqual([["mobile", "profile-change-requests"]]);
  });

  it("refreshes the notifications inbox, bootstrap, and facets when a notification row changes", () => {
    expect(
      getMobileRealtimeInvalidationKeys("token-1", "notifications"),
    ).toEqual([
  ...
  it("refreshes schedule + requests when recurring shifts change", () => {
    expect(
      getMobileRealtimeInvalidationKeys("token-1", "recurring_shifts"),
    ).toEqual([["mobile", "schedule"], ["mobile", "requests"]]);
  });

  it("refreshes schedule on publish history changes", () => {
    expect(
      getMobileRealtimeInvalidationKeys("token-1", "publish_history"),
    ).toEqual([["mobile", "schedule"]]);
  });

  it("refreshes bootstrap + profile for audit-log and impersonation events", () => {
    expect(
      getMobileRealtimeInvalidationKeys("token-1", "audit_log"),
    ).toEqual([
      ["mobile", "bootstrap", "token-1"],
      ["mobile", "profile", "token-1"],
    ]);
    expect(
      getMobileRealtimeInvalidationKeys("token-1", "impersonation_sessions"),
    ).toEqual([
      ["mobile", "bootstrap", "token-1"],
      ["mobile", "profile", "token-1"],
    ]);
