...
export function buildNavGroups(
  ...
  if (perms.canAccessSettings) {
    ...
    if (perms.isSuperAdmin) generalItems.push({ id: "org-general", label: "Organization Details", Icon: BuildingIcon, description: "Manage your organization's name and basic profile information." });
    if (perms.isSuperAdmin || perms.canManageOrgLabels || perms.canViewOrgLabels) generalItems.push({ id: "org-labels", label: "Labels", Icon: LabelsIcon, helpHint: "Labels rename visible wording across DubGrid but do not change behavior.", description: "Customize the terminology used in your organization. For example, rename 'Focus Areas' to 'Wings' or 'Units'." });
  ...
  if (perms.canAccessSettings) {
    ...
    if (perms.canManageOrgSettings) schedItems.push({ id: "org-display", label: "Shift Display Mode", Icon: DisplayIcon, helpHint: "Choose between readable full names and denser short shift codes.", description: "Choose how shifts appear on the schedule grid: short codes or full names." });
    if (perms.isSuperAdmin) schedItems.push({ id: "schedule-rules", label: "Schedule Rules", Icon: ShieldIcon, description: "Configure automated scheduling rules and constraints." });
    if (perms.canManageScheduleDefinitions || perms.canViewScheduleDefinitions) schedItems.push({ id: "schedule-shifts", label: "Shifts", Icon: LabelsIcon, description: "Configure your core Day, Evening, Night, and similar shift definitions with default times and colors." });
    if (perms.canManageScheduleDefinitions || perms.canViewScheduleDefinitions) schedItems.push({ id: "schedule-jobs", label: "Jobs", Icon: CalendarIcon, helpHint: "Jobs can be scheduled or general and can restrict qualification.", description: "Define responsibilities like Supervisor, Mentor, Nurse, and Office, including assignment rules and grid visibility." });
    if (perms.canManageScheduleDefinitions || perms.canViewScheduleDefinitions) schedItems.push({ id: "schedule-absence-types", label: "Absence Types", Icon: CalendarIcon, helpHint: "Absences replace worked assignments and are excluded from coverage.", description: "Manage PTO, sick, vacation, calloff, and other non-worked schedule labels." });
  ...
  if (perms.isSuperAdmin || perms.isGridmaster) {
    groups.push({
      ...
      label: "Billing",
      items: [
        { id: "org-billing", label: "Subscription", Icon: BillingIcon, helpHint: "Seats are based on active organization users, not employees.", description: "Review subscription status, seats, and Stripe billing access." },
      ],
    });
  }
  ...
  if (perms.isSuperAdmin) {
    groups.push({
      ...
      label: "Audit",
      items: [
        { id: "org-activity", label: "Activity Log", Icon: ActivityIcon, helpHint: "A record of changes made across your organization.", description: "Review the audit trail of role, membership, billing, and configuration changes across your organization." },
      ],
    });
  }
  ...
  if (perms.isGridmaster) {
    groups.push({
      ...
      label: "Platform",
      items: [
        { id: "platform-impersonation", label: "Impersonation", Icon: ImpersonateIcon, description: "Temporarily access an organization's account as any of its users for support or debugging." },
      ],
    });
  }
  ...
  if (perms.isSuperAdmin) {
    groups.push({
      ...
      label: "Danger Zone",
      items: [
        { id: "org-danger", label: "Delete Organization", Icon: DangerIcon, helpHint: "This cancels billing and removes access for everyone.", description: "Permanently close this organization. Billing stops and all members lose access." },
      ],
    });
  }
