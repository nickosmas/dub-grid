...
import { STARTUP_SURFACE_DELAY_MS } from "@dubgrid/design-tokens";
...
describe("ProtectedRoute", () => {
  ...
  it("shows the branded startup surface once a session restore outlasts the flicker threshold", async () => {
    ...
    await act(async () => {
      await vi.advanceTimersByTimeAsync(STARTUP_SURFACE_DELAY_MS);
    });
  ...
  it("paints nothing for a restore that resolves inside the flicker threshold", async () => {
    ...
    await act(async () => {
      await vi.advanceTimersByTimeAsync(STARTUP_SURFACE_DELAY_MS - 1);
    });
    expect(screen.queryByRole("progressbar")).not.toBeInTheDocument();
  ...
  it("stays blank while redirecting an unauthenticated visitor", async () => {
    ...
    await act(async () => {
      await vi.advanceTimersByTimeAsync(STARTUP_SURFACE_DELAY_MS * 4);
    });
