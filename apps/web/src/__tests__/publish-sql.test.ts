...
describe("publish schedule database contract", () => {
  ...
  it("validates open-shift volunteering against current coverage requirements", () => {
    ...
    expect(sql).toContain("p_is_mentored_flags   BOOLEAN[] DEFAULT '{}'::BOOLEAN[]");
    ...
    expect(sql).toContain("RAISE EXCEPTION 'You already volunteered for this open shift'");
    expect(sql).toContain(
      "IF v_actual_staff + COALESCE(v_pending_volunteer_count, 0) >= v_required_staff THEN",
    );
    expect(sql).toContain("RAISE EXCEPTION 'That open shift is no longer available'");
    expect(sql).toContain("COALESCE(p_is_mentored_flags, '{}'::BOOLEAN[])");
    expect(sql).toContain("'focusAreaId', p_focus_area_id");
    expect(sql).toContain("v_focus_area_id := NULLIF(p_state->>'focusAreaId', '')::BIGINT;");
    expect(sql).toContain("snapshot.focus_area_id IS NOT DISTINCT FROM p_focus_area_id");
    expect(sql).toContain(
      "snapshot.focus_area_id IS NOT DISTINCT FROM v_requester_state.focus_area_id",
    );
    expect(sql).toContain("snapshot.focus_area_id IS NOT DISTINCT FROM other_state.focus_area_id");
  });
});
