...
vi.mock("@/features/account/server", () => ({
  fetchImportantUserSessionsForUser: (userId: string) => fetchImportantUserSessionsForUser(userId),
...
describe("/api/account/sessions", () => {
  ...
  it("loads only the important (active + last-inactive) sessions for the signed-in user", async () => {
    const response = await GET(new NextRequest("http://localhost/api/account/sessions"));

    expect(response.status).toBe(200);
    expect(fetchImportantUserSessionsForUser).toHaveBeenCalledWith("user-id");
  ...
  it("rejects unauthenticated session reads", async () => {
    ...
    expect(fetchImportantUserSessionsForUser).not.toHaveBeenCalled();
  });
