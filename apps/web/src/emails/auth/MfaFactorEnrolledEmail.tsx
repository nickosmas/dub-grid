...
export function MfaFactorEnrolledEmail({ logoUrl }: { logoUrl?: string } = {}) {
  return (
    <SecurityNotificationEmail
      ...
      body="An authenticator app was added as a verification method for your DubGrid account."
      // A reset would ask for the new authenticator, which may be someone
      // else's, so the only useful step for the owner is support.
      footer="If it was you, you don't need to do anything. If it wasn't you, contact support@dubgrid.com right away, since someone else may be using your DubGrid account."
      logoUrl={logoUrl}
    />
  );
}
