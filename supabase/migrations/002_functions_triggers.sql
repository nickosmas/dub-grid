...
-- ── scheduler-staffed calloff finalization ─────────────────────────────────
-- A scheduler assignment remains an ordinary draft until publish_schedule
-- inserts its per-cell change row. Resolving the matching calloff pickup from
-- this AFTER INSERT trigger keeps both state transitions in the same database
-- transaction and leaves discarded drafts completely untouched.
...
BEGIN
  ...
  -- Keep only segment occurrences newly added by this publish. row_number makes
  -- this a multiset subtraction, so duplicate assignment pairs remain exact.
  WITH to_segments AS (
    SELECT
      segment.shift_id,
      segment.job_id,
