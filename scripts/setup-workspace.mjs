  ...
  try {
    ...
    if (fetchError) {
      ...
      if (insertError) {
        ...
        console.log("Please run the following SQL in your Supabase SQL Editor:");
        console.log(
          `INSERT INTO public.organizations (name, slug, address, phone) VALUES ('Main Facility', 'main-facility', '123 Care St', '555-0123') RETURNING id;`,
        );
      } else {
        ...
        if (linkError) {
          ...
          console.log(
            `SELECT public.assign_org_role_by_email('${email}', '${newOrg.id}', 'admin');`,
          );
        } else {
