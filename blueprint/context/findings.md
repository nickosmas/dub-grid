...
### F-04 [P2] closed - Mobile People renders an unvirtualized full roster
...
### F-17 [P2] closed - Mobile dashboard remains an explicitly reduced analytics model
...
### F-30 [P2] closed - Most authenticated mobile queries use the rotating token as cache identity
...
### F-31 [P2] closed - Paginated mobile alerts accumulate in an unvirtualized ScrollView
...
### F-42 [P2] closed - MembersSection tests finish with unwrapped responsive updates
...
### F-45 [P2] closed - Tablet user dashboard mixes a stacked shell with a desktop top grid
...
### F-46 [P3] closed - Three parallel button systems, two of them near-dead
...
### F-47 [P3] closed - Radius values largely bypass the radius tokens
...
### F-48 [P3] closed - Icon sources are split between lucide and hand-rolled SVG
...
### F-49 [P3] closed - Em dashes in user-visible copy, against the writing standard
...
### F-50 [P2] closed - Confirm-button capitalization is split between two conventions
...
### F-51 [P2] closed - 95 direct `process.env` reads against the validated-env rule
...
### F-52 [P3] closed - 33 dead `eslint-disable` directives
...
### F-53 [P2] closed - Typography contract test is failing on `dev`
...
### F-55 [P2] closed - Delete dependency checks read every schedule cell in the org
...
### F-56 [P2] closed - Web confirmations allow alternate dismissal while their action is pending
...
### F-57 [P2] closed - Management confirmation failures render their error behind the active popup
...
### F-58 [P2] closed - Shared web Modal does not restore focus after closing
...
### F-59 [P2] closed - Mobile overlay instructions contradict the current confirmation primitive
...
### F-60 [P2] closed - The modal depth guard no longer enforces its documented sheet-plus-confirmation limit
...
### F-61 [P2] closed - Confirmation dismissal test passes against a control name the component no longer uses
...
### F-65 [P2] closed - Sticky date row's ARIA rows and column headers have no owning grid
...
### F-66 [P3] closed - Search scroll margin hard-codes the sticky group's height
...
### F-67 [P3] closed - Read-only shift detail panel is announced as "Edit shift"
...
### F-68 [P3] closed - Shell renders nothing while the organization bootstrap retries a 5xx
...
### F-69 [P3] closed - Impersonation end accepts any reason, then answers a constraint violation with a 500
...
### F-70 [P3] closed - Starting an impersonation while one is active loses the RPC's message behind a 500
...
### F-71 [P3] closed - Impersonation banner names the target while the shell greets the gridmaster
...
### F-72 [P3] closed - The /gridmaster safety escape clears the cookie but leaves the impersonation row active
...
### F-73 [P2] closed - Ending impersonation sometimes bounces the gridmaster to /login on Firefox
...
### F-74 [P3] closed - The Gridmaster route's not-found boundary is unreachable
...
### F-77 [P2] closed - A request sheet opened during another sheet's dismissal is dropped by UIKit and stays "open" in JS
...
### F-82 [P2] closed - A production 404 under the app renders with every script blocked by the nonce CSP
...
### F-83 [P1] closed - Gridmaster organization creation answers 500 unless a time zone was picked
...
### F-84 [P1] closed - The gridmaster "Send password reset" never sends an email, then reports success and audits it as sent
...
### F-85 [P1] closed - The org.created audit row persists the raw super-admin invitation token
...
### F-86 [P1] closed - Deactivating a user from the portal leaves their issued tokens valid
...
### F-87 [P2] closed - Members of a suspended or deleted organization are never told why they are locked out
...
### F-88 [P2] closed - Gridmaster oversight counts sandbox clones as tenants
...
### F-89 [P2] closed - Oversight fact loading reads whole tables and is silently capped at 1000 rows each
...
### F-90 [P2] closed - The gridmaster invitations list ships every invitation's raw token to the browser
...
### F-91 [P2] closed - Gridmaster archive cancels Stripe from the browser, unmentioned and unmatched by the API
...
### F-92 [P2] closed - A scheduled department with no focus area is a dead end in the Structure editor
...
### F-93 [P2] closed - send-invite-email mails any address a branded invitation with caller-supplied token and copy
...
### F-94 [P3] closed - Structure step buttons read "+ Add add a role..." and "+ Add add a certification..."
...
### F-95 [P3] closed - Gridmaster archive, restore, suspend and unsuspend never invalidate the org caches
...
### F-96 [P3] closed - The "Organization Created" screen misstates the invited role and uses an em dash
...
### F-98 [P3] closed - lib/db/admin.ts is a dead gridmaster data layer
...
### F-99 [P3] closed - assign_org_role_by_email matches the email case-sensitively
...
### F-100 [P3] closed - Wizard job-editor copy clips and the display-mode sample overflows the column
...
### F-101 [P3] closed - Create Organization wizard fields have no label association
...
### F-102 [P0] closed - Gridmaster deactivation did not stop an open web session
...
### F-103 [P2] closed - An organization id the portal cannot resolve rendered a blank pane
...
### F-104 [P2] closed - A member whose organization row is no longer visible was sent to the billing gate
...
### F-105 [P3] fixed - Two unused eslint-disable directives have crept back in
...
**Resolution:** Fixed 2026-09-20: both directives removed; `npm run lint` reports 0 unused directives (3 `no-img-element` warnings remain, unrelated). Requires `/audit` re-review before closing.

### F-106 [P3] fixed - Em dashes remain in four user-visible strings
...
**Resolution:** Fixed 2026-09-20: the five strings use a colon or comma; no test pinned the old copy. Requires `/audit` re-review before closing.

### F-107 [P3] fixed - Oversight facts memo outlives a lifecycle change by up to ten seconds
...
**Resolution:** Fixed 2026-09-20: `resetOversightFactsMemo()` exported and called from `invalidateOrganizationAccess`, which every lifecycle branch runs; test asserts a second load re-reads after the reset. Requires `/audit` re-review before closing.
