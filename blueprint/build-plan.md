...
> `/feature` finds the next unchecked item. Items 1 through 39 are checked as
> of 2026-09-23, when migrations 026 to 040 were applied to production and the
> production migration gate closed. Item 41 is the next release-candidate work;
> item 40 is intentionally deferred until the authentication remediation closes.
...
- [x] 4. **Real-time schedule collaboration** - Supabase Realtime editor
      presence and optimistic version checks (advisory cell locks were
      tried and removed)
...
- [x] 37. **Production migration safety** - final release gate only after all
      ...
      completed gate does not authorize later production changes. Local safety
      tooling and qualification are preserved in checkpoint `ee4207e0`.
...
- [ ] 40. **Documentation truth and drift prevention** - make the next `dev`
      release candidate the source of truth for public and internal
      documentation. Every customer-facing web route, mobile screen, Settings
      section, role or platform variance, and meaningful state must be
      documented or explicitly excluded in a machine-checked coverage
      manifest. Preserve historical records as point-in-time evidence, keep
      internal operations out of public guides, and make future source drift a
      failing verification condition. No product API, database schema, or UI
      behavior changes are included.
  - [ ] 40a. **Source-derived inventory and documentation contract** - generate
        deterministic inventories for routes, settings panels, API methods,
        permissions, workspaces, migrations, RLS tables, environment keys,
  ...
  - [ ] 40d. **Current engineering, architecture, security, and operations
        ...
        source paths; regenerate the Blueprint overview and frame historical
        audits as point-in-time evidence where needed.
  - [ ] 40e. **Cross-platform runtime qualification and permanent drift
        gates** - exercise the documented web role and state matrix in
        Chromium, Firefox, and WebKit, record iOS and Android mobile evidence,
