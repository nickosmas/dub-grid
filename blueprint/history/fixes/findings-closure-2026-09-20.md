# Findings closure re-review, 2026-09-20

**Type:** Fix (audit re-review and ledger archival)

**Status:** verified

## What this records

Every repaired finding in the ledger was re-reviewed against the repaired
code by `/audit` on 2026-09-20 and closed: the P0/P1 set (F-102, F-83, F-84,
F-85, F-86), this session's repairs (F-04, F-17, F-31, F-42, F-45, F-55,
F-67 to F-74, F-87, F-89 to F-95, F-100, F-101), the four small findings the
re-review itself raised (F-97 reopened, F-105 to F-107), and the fixed
entries carried forward from earlier work (F-30, F-46 to F-53, F-56 to F-61,
F-65, F-66, F-77, F-82, F-88, F-96, F-98, F-99, F-103, F-104).

The repairs themselves are logged under their own archives in this folder
(one-click-calendar-subscription, gridmaster-password-reset-and-audit-token,
gridmaster-impersonation-findings, gridmaster-lifecycle-findings,
setup-wizard-findings, web-findings-batch-2026-09-20,
mobile-virtualized-lists-and-daily-coverage) and in the earlier feature
archives their **Found** lines name.

## Findings

### findings-closure-2026-09-20/F-04 [P2] closed - Mobile People renders an unvirtualized full roster
...
shared scroll architecture and needs its own spec, not a cleanup pass. Closed 2026-09-20 by `/audit` re-review: Screen list mode reviewed (FlatList keeps insets, refresh control, scroll handle; header fills when empty); People suites and Screen list tests pass; simulator shows the roster and the collapsing large title.

### findings-closure-2026-09-20/F-17 [P2] closed - Mobile dashboard remains an explicitly reduced analytics model
...
**Resolution:** Fixed 2026-09-20. Alerts render through `Screen`'s new `list` mode: search, filter strip and unread row stay as the header, rows mount only near the viewport, "Load more" is the list footer, and the hairline separators and gutter bleed are unchanged. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Alerts render through Screen list mode; separators, gutter bleed and Load more footer confirmed on the simulator; 50 notification tests pass.

### findings-closure-2026-09-20/F-42 [P2] closed - MembersSection tests finish with unwrapped responsive updates
...
`useMediaQuery`, so the original timing defect remains open. Closed 2026-09-20 by `/audit` re-review: 31 MembersSection tests, 0 act warnings on this pass.

### findings-closure-2026-09-20/F-45 [P2] closed - Tablet user dashboard mixes a stacked shell with a desktop top grid
...
**Resolution:** Fixed 2026-09-05 in ec9c7a10. Measuring the literals showed the scale was missing a step rather than being ignored: the most common value was 4px at 76 sites with no token, so `--dg-radius-xs` was added to name what already existed. 239 literals now resolve through tokens (4/6/8/10/12 to xs/sm/md/lg/xl), values identical so visually a no-op. Left raw on purpose: `999`/`9999`/`50%` are shapes not scale steps, and the off-scale one-offs (3, 5, 7, 9, 14, 16, 20) are decisions a sweep should not silently normalize. A lint rule to hold the line is still worth adding. Closed 2026-09-20 by `/audit` re-review: `--dg-radius-xs` present in globals.css.

### findings-closure-2026-09-20/F-48 [P3] closed - Icon sources are split between lucide and hand-rolled SVG
...
**Resolution:** Fixed 2026-09-05 in 07bac5fd. 62 of the 176 were hand-copied lucide paths and now use the components, preserving each site's size, stroke width and colour. 176 -> 113 inline SVGs. Closed 2026-09-20 by `/audit` re-review: Inline SVG now in 59 files (from 68); lucide components in use at the converted sites.
...
### findings-closure-2026-09-20/F-49 [P3] closed - Em dashes in user-visible copy, against the writing standard
...
**Resolution:** Fixed 2026-09-05. Decided on evidence rather than preference: multi-word UI text across the app ran 124 sentence case to 39 Title Case, so the code had already chosen, and several dialog trigger buttons were sentence case while only their confirms were Title Case. Normalized 22 label literals across 11 files to sentence case (which also resolved the `Change Role` / `Change role` duplicate), and corrected the copy tone guide, whose Title Case rule was the stale half. Four tests drove these buttons by accessible name and needed scoping to their dialog, because once a trigger and its confirm share a label `getByRole` matches both. Closed 2026-09-20 by `/audit` re-review: No Title Case multi-word `confirmLabel` literals remain.

### findings-closure-2026-09-20/F-51 [P2] closed - 95 direct `process.env` reads against the validated-env rule
...
### findings-closure-2026-09-20/F-52 [P3] closed - 33 dead `eslint-disable` directives
...
**Resolution:** Fixed 2026-09-05 with `eslint src --fix-type directive --fix`, which by construction only edits directive comments. Count went 33 to 0. Closed 2026-09-20 by `/audit` re-review: The 33 listed directives are gone. Two newer unused directives elsewhere are recorded as F-105.

### findings-closure-2026-09-20/F-53 [P2] closed - Typography contract test is failing on `dev`
...
**Resolution:** Fixed 2026-09-05 by documenting the exception. The 9px is deliberate and the code comment says why: two initials at badge size run wider than the 20px marker circle, so the marker sizes its text off the ring. Tokenising it would reintroduce the overflow, so `components/ScheduleGrid.tsx: 1` was added to `documentedMicroTextCounts` instead. The typography suite is green again. Closed 2026-09-20 by `/audit` re-review: Typography suite: 24 pass.

### findings-closure-2026-09-20/F-55 [P2] closed - Delete dependency checks read every schedule cell in the org
...
**Resolution:** Fixed by the user-approved confirmation-surfaces implementation: all web confirmation exits share the pending guard, the redundant X is removed, and both primary and secondary async latches remain effective even if an external loading flag is false. Regression tests cover Escape, backdrop, Cancel, and dismissal after settlement. Closed 2026-09-20 by `/audit` re-review: ConfirmDialog/Modal tests (24) pass; pending guard and `isLoading` latch present.

### findings-closure-2026-09-20/F-57 [P2] closed - Management confirmation failures render their error behind the active popup
...
**Resolution:** Fixed: remove-access and invitation errors render in the active ManagementUserActionsSheet confirmation and clear on cancellation/retry. Three rejected-action tests assert the error within the visible alert and its reset on reopening. PersonDetail invitation/account-link errors also remain on the active surface; handoff uses the existing transition helper. Native visual validation remains outstanding. Closed 2026-09-20 by `/audit` re-review: Errors render inside the active ManagementUserActionsSheet confirmation; the sheet's tests pass.

### findings-closure-2026-09-20/F-58 [P2] closed - Shared web Modal does not restore focus after closing
...
**Resolution:** Fixed: both app guides now describe consequence confirmations, task sheets, and page workflows, including pending/error ownership, action order, focus return, and explicit gate exceptions. Mobile confirmation motion and icon treatment are quieter, action layout adapts to narrow/enlarged text, status choices/reasons moved to a sheet, and ordinary profile saves no longer add a redundant confirmation. Closed 2026-09-20 by `/audit` re-review: apps/mobile/AGENTS.md describes one task sheet plus at most one confirmation with `useModalHandoff`.

### findings-closure-2026-09-20/F-60 [P2] closed - The modal depth guard no longer enforces its documented sheet-plus-confirmation limit
...
**Resolution:** Fixed at the presentation-policy/test layer: both real primitives register typed presentations with identity-based cleanup. Duplicate task sheets and duplicate confirmations are diagnosed; explicit required gates are separate. Regression tests render the actual ConfirmationModal above a BottomSheetModal, reject invalid compositions, and cover cleanup/handoff. This remains a diagnostic guard, not a native modal coordinator; iOS/Android transition and gesture checks remain outstanding. Closed 2026-09-20 by `/audit` re-review: `registerModalPresentation` in place; presentation tests (12) pass.

### findings-closure-2026-09-20/F-61 [P2] closed - Confirmation dismissal test passes against a control name the component no longer uses
...
**Resolution:** Fixed: tests use the real Dismiss control and capture React Native Modal onRequestClose. Idle dismissal calls Cancel without invoking the action; pending backdrop, Cancel, Android back, and duplicate confirmation are covered. These are component tests with native shims, not device evidence. Closed 2026-09-20 by `/audit` re-review: Test targets the real Dismiss control and `onRequestClose`; passes.
...
### findings-closure-2026-09-20/F-65 [P2] closed - Sticky date row's ARIA rows and column headers have no owning grid
...
**Resolution:** Repaired 2026-09-07 in the same session: the section wrapper is now the `role="grid"` with the section label, and the header and body containers are `role="rowgroup"`. A grid test asserts one grid owns both rowgroups. Closed 2026-09-20 by `/audit` re-review: `role="grid"` on the section with two `role="rowgroup"` children.

### findings-closure-2026-09-20/F-66 [P3] closed - Search scroll margin hard-codes the sticky group's height
...
**Resolution:** Repaired 2026-09-07 in the same session: the geometry effect publishes `--dg-grid-sticky-height` on the section and the row scroll margin reads it, keeping `80px` only as the fallback. Closed 2026-09-20 by `/audit` re-review: `--dg-grid-sticky-height` published and read (3 references).

### findings-closure-2026-09-20/F-67 [P3] closed - Read-only shift detail panel is announced as "Edit shift"
...
**Resolution:** Fixed 2026-09-20. The dialog label is `allowShiftEdits ? "Edit shift" : "Shift details"`; unit test covers both, and `e2e/role-variance.spec.ts` now matches the read-only panel by its accessible name. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Dialog label derives from `allowShiftEdits`; unit test covers both names; role-variance e2e passes on Chromium with the accessible-name matcher.

### findings-closure-2026-09-20/F-68 [P3] closed - Shell renders nothing while the organization bootstrap retries a 5xx
...
**Resolution:** Fixed 2026-09-20. `SetupGuard` renders `AuthTransitionScreen` (workspace phase) instead of `null` while the bootstrap is loading with nothing cached, so a retrying 5xx shows progress rather than an empty shell; new `SetupGuard.test.tsx`. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: SetupGuard renders the workspace transition while loading cold; SetupGuard tests pass; the bootstrap-state e2e now waits the retry window for Try again (product behaviour unchanged, the heading simply appears earlier).

### findings-closure-2026-09-20/F-69 [P3] closed - Impersonation end accepts any reason, then answers a constraint violation with a 500
...
**Resolution:** Fixed 2026-09-20. `endSchema.reason` is `z.enum(["manual", "expired", "navigation"])`, so an unknown value is a 400 before the RPC; route test covers `probe` (400, no RPC) and `navigation` (200). Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: `endSchema.reason` is the constraint enum; route tests pass.

### findings-closure-2026-09-20/F-70 [P3] closed - Starting an impersonation while one is active loses the RPC's message behind a 500
...
**Resolution:** Fixed 2026-09-20 by deleting `(app)/gridmaster/not-found.tsx`: nothing under the segment calls `notFound()` and the standards forbid a catch-all child, so the boundary could never render. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: `(app)/gridmaster/not-found.tsx` is gone.

### findings-closure-2026-09-20/F-77 [P2] closed - A request sheet opened during another sheet's dismissal is dropped by UIKit and stays "open" in JS
...
**Resolution:** Fixed 2026-09-19: the root `not-found.tsx` awaits `connection()` so Next renders it per request and stamps the nonce it reads from the request CSP header; `next build` now lists `/_not-found` as dynamic. Verified on a production build (`next build` + `next start`) with the gridmaster not-found spec reporting no unexpected console failures; the CI e2e run on the release PR is the second proof. Closed 2026-09-20 by `/audit` re-review: `not-found.tsx` awaits `connection()`.

### findings-closure-2026-09-20/F-83 [P1] closed - Gridmaster organization creation answers 500 unless a time zone was picked
...
**Resolution:** Fixed 2026-09-19: `loadOversightFacts` selects `workspace_kind = real` through `selectRealOrganizations`, with a test asserting the filter. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: `selectRealOrganizations` filters `workspace_kind = real`; test asserts it.

### findings-closure-2026-09-20/F-89 [P2] closed - Oversight fact loading reads whole tables and is silently capped at 1000 rows each
...
**Resolution:** Fixed 2026-09-20. `token` dropped from the gridmaster invitations select and from the client types; new route test asserts the column list excludes it. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: The invitations select has no `token` column; route test asserts it.

### findings-closure-2026-09-20/F-91 [P2] closed - Gridmaster archive cancels Stripe from the browser, unmentioned and unmatched by the API
...
**Resolution:** Fixed 2026-09-20. `StringListSettings` derives the add button from `label` (singular) with an explicit `addLabel` override, so it reads "+ Add Role" / "+ Add Certification"; the placeholder stays input hint copy. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Add button derives from `label`; settings suites pass.

### findings-closure-2026-09-20/F-95 [P3] closed - Gridmaster archive, restore, suspend and unsuspend never invalidate the org caches
...
**Resolution:** Fixed 2026-09-20. Archive, restore, suspend and unsuspend all drop `mwOrgAccess`, `organization` and `orgBySlug`; the route test that codified the gap now asserts the keys. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: All four lifecycle branches call `invalidateOrganizationAccess` with the slug; tests assert the keys.

### findings-closure-2026-09-20/F-96 [P3] closed - The "Organization Created" screen misstates the invited role and uses an em dash
...
**Resolution:** Fixed 2026-09-19: `platform_feature_flags.` added to the oversight high-risk prefixes and to the `get_filtered_audit_log` allow-list in migration 021. Requires `/audit` re-review before closing. Reopened 2026-09-20 by `/audit` re-review: migration 021 lists `platform_feature_flags.%` in the audit-log allow-list, but `HIGH_RISK_ACTION_PREFIXES` in `apps/web/src/app/api/gridmaster/_lib/oversight.ts:22` still ends at `impersonation.`, so kill-switch changes are not classified as high-risk in Security oversight. Half of the repair is missing. Fixed 2026-09-20: `platform_feature_flags.` added to `HIGH_RISK_ACTION_PREFIXES`; the activity-summary test now includes a flag update and expects it counted as review-worthy. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Prefix present in `HIGH_RISK_ACTION_PREFIXES`; activity-summary test counts a flag update as review-worthy.

### findings-closure-2026-09-20/F-98 [P3] closed - lib/db/admin.ts is a dead gridmaster data layer
...
**Resolution:** Fixed 2026-09-19: `lib/db/admin.ts` deleted and its barrel export removed; type-check clean. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: `lib/db/admin.ts` no longer exists.

### findings-closure-2026-09-20/F-99 [P3] closed - assign_org_role_by_email matches the email case-sensitively
...
**Resolution:** Fixed 2026-09-19 in migration 021: `assign_org_role_by_email` matches `lower(email)` and also revives an archived membership on conflict, which is what lets a reinstated or previously removed person back in. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Migration 021 matches `lower(email)` in `assign_org_role_by_email`.

### findings-closure-2026-09-20/F-100 [P3] closed - Wizard job-editor copy clips and the display-mode sample overflows the column
...
**Resolution:** Fixed 2026-09-20. Name, the three label inputs, and the super admin first name, last name, email and phone inputs carry `useId` ids with `htmlFor` labels; test asserts each via `getByLabelText`. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Eight wizard inputs carry `useId` ids with `htmlFor` labels; test uses `getByLabelText`.

### findings-closure-2026-09-20/F-102 [P0] closed - Gridmaster deactivation did not stop an open web session
...
**Resolution:** Fixed 2026-09-19 exactly that way (migration 021, `permissions.ts`, `users/route.ts`), with a middleware test, a permissions test for both deactivated and terminated callers, and a users-route test for the revocation. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Same evidence as F-86; the P0 path (hook refusal at issue, permissions refusal for held tokens, watermark on deactivate) is present and tested.

### findings-closure-2026-09-20/F-103 [P2] closed - An organization id the portal cannot resolve rendered a blank pane
...
**Resolution:** Fixed 2026-09-19: the portal shows the loading bar or the empty state, and oversight labels missing organizations "Deleted organization <id>" instead of an 8-character prefix. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Oversight labels unresolved organizations "Deleted organization <id>"; portal renders the loading bar or empty state.

### findings-closure-2026-09-20/F-104 [P2] closed - A member whose organization row is no longer visible was sent to the billing gate
...
Closed 2026-09-20 by `/audit` re-review: Proxy redirects to /login when the org row is invisible (`orgAccess === null`); middleware test passes.

### findings-closure-2026-09-20/F-105 [P3] closed - Two unused eslint-disable directives have crept back in

**File:** apps/web/next.config.ts:112; apps/mobile/src/features/schedule/screens/ScheduleScreen.test.tsx:2273
**Found:** 2026-09-20 by /audit (scope: fixed-findings re-review; lens: quality)
**Why it matters:** `npm run lint` reports both as unused (`no-require-imports`, `no-bitwise`). Neither is from F-52's original set; both arrived later. Warnings only, but they are the same drift F-52 cleared.
**Suggested fix:** Remove the two comments.
**Resolution:** Fixed 2026-09-20: both directives removed; `npm run lint` reports 0 unused directives (3 `no-img-element` warnings remain, unrelated). Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: Both directives gone; repo lint reports 0 unused directives.

### findings-closure-2026-09-20/F-106 [P3] closed - Em dashes remain in four user-visible strings

**File:** apps/web/src/app/(app)/onboarding/page.tsx:157; apps/web/src/components/ShiftRequestBoard.tsx:330; apps/web/src/components/PublishHistoryPanel.tsx:771; apps/web/src/components/PrintScheduleView.tsx:1096,1272
**Found:** 2026-09-20 by /audit (scope: fixed-findings re-review; lens: quality)
**Why it matters:** The writing standard bans U+2014 in generated content; F-49 cleared the sites it listed, and these are newer or were outside its list ("I've been invited — refresh", "{label} — {name}", "{range} — {n} changes", "Schedule — {range}").
**Suggested fix:** Replace with a colon, comma, or hyphen as each reads best.
**Resolution:** Fixed 2026-09-20: the five strings use a colon or comma; no test pinned the old copy. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: No U+2014 remains in the five strings; the remaining glyphs in those files are the empty-value placeholder and code comments.

### findings-closure-2026-09-20/F-107 [P3] closed - Oversight facts memo outlives a lifecycle change by up to ten seconds

**File:** apps/web/src/app/api/gridmaster/_lib/oversight.ts (oversightFactsMemo); apps/web/src/app/api/gridmaster/organizations/manage/route.ts (archive, restore, suspend, unsuspend)
**Found:** 2026-09-20 by /audit (scope: fixed-findings re-review; lens: quality)
**Why it matters:** F-89's memo shares one facts load across the portal's parallel views for 10 s, but the lifecycle route drops only the Redis caches. A gridmaster who archives or suspends an organization and immediately refreshes the overview can see the old state for the rest of the window.
**Suggested fix:** Export a `resetOversightFactsMemo()` from oversight.ts and call it from the four lifecycle branches.
**Resolution:** Fixed 2026-09-20: `resetOversightFactsMemo()` exported and called from `invalidateOrganizationAccess`, which every lifecycle branch runs; test asserts a second load re-reads after the reset. Requires `/audit` re-review before closing. Closed 2026-09-20 by `/audit` re-review: `invalidateOrganizationAccess` resets the memo; test asserts the re-read.
