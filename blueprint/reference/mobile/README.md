...
| alerts-ios-light-default-admin                                                               | captured | Mailbox rows with a title-over-content hierarchy: semibold row title (bold when unread), the full message a step down in size and color, the time a step further; unread rows tinted brandSoft; tapping a row goes to what it is about                 |
| alerts-ios-dark-ax-admin                                                                     | captured | The same rows in dark at accessibility extra-large: nothing truncates, the hierarchy holds at large text                                                                                                                                               |
| dashboard-activity-ios-light-default-admin                                                   | captured | Recent activity: one row per publish with its change summary, no per-cell rows, request statuses and roles as copy                                                                                                                                     |
| alerts-swipe-ios-light-default-admin                                                         | captured | A row swiped left as a rounded card sliding off the edge, with round Read and Archive buttons and their labels on the page behind it, as iOS Mail lays them out                                                                                        |
| alerts-swipe-ios-dark-ax-admin                                                               | captured | The same reveal in dark at accessibility extra-large: the card fill is the dark surface, the labels are capped so the two slots stay apart                                                                                                             |
