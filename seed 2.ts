...
// Load env file: --remote flag loads .env.remote, otherwise .env.local
...
async function main() {
  const connString = process.env.DATABASE_URL ?? "postgresql://postgres:postgres@127.0.0.1:54322/postgres";
  const isRemote = connString.includes("supabase");
  const dbConfig = { connectionString: connString, ssl: isRemote ? { rejectUnauthorized: false } : undefined };
  const db = new Client(dbConfig);
  await db.connect();

  console.log(`Using ${envFile} → ${isRemote ? "REMOTE" : "LOCAL"} database`);
