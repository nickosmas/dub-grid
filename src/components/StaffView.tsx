...
export default function StaffView({ employees, wings, onSave, onDelete, onAdd }: StaffViewProps) {
    ...
    () =>
      [...employees].sort((a, b) => {
        switch (sortBy) {
          ...
          default: return a.seniority - b.seniority;
        }
      }),
    [employees, sortBy],
  );
  ...
  const handleDelete = useCallback((empId: number) => {
    onDelete(empId);
    setExpandedEmpId(null);
  }, [onDelete]);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            {(["seniority", "name", "wing"] as const).map((option) => (
              <button
                ...
                style={{
                  ...
                  borderRight: option !== "wing" ? "1px solid var(--color-border)" : "none",
                }}
              >
                {option === "seniority" ? "Seniority" : option === "name" ? "Name" : "Assigned Wings"}
              </button>
            ))}
          </div>
        </div>
      ...
      >
        ...
        >
          {["#", "Name", "Assigned Wings", "Skill Level", "Roles", ""].map((h, i) => (
            <div
              key={i}
              style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-subtle)", letterSpacing: "0.06em" }}
            >
              {h}
            </div>
          ))}
          ...
          return (
            <div key={emp.id}>
              ...
              <div
                ...
                onMouseEnter={(e) => {
                  if (!isExpanded) e.currentTarget.style.background = "var(--color-row-hover, rgba(0,0,0,0.02))";
                }}
                onMouseLeave={(e) => {
                  if (!isExpanded) e.currentTarget.style.background = i % 2 === 0 ? "#fff" : "var(--color-row-alt)";
                }}
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <div
                    style={{
                      ...
                      background: `hsl(${(emp.id * 47) % 360}, 55%, 88%)`,
                      display: "flex", alignItems: "center", justifyContent: "center",
                      fontSize: 12, fontWeight: 700,
                      color: `hsl(${(emp.id * 47) % 360}, 55%, 35%)`,
                      flexShrink: 0,
                    }}
                  >
                  ...
                  <div>
                    <div
                      style={{
                        fontWeight: 600, fontSize: 14, color: "var(--color-text-secondary)",
                    ...
                    >
                      ...
                      {emp.fteWeight < 1 && (
                        <span
                          style={{
                            fontSize: 10, fontWeight: 700, background: "var(--color-border-light)",
                            color: "var(--color-text-muted)", padding: "1px 4px", borderRadius: 4,
                          }}
                        >
                          {emp.fteWeight}
                        </span>
                  ...
                  {emp.wings.map((wing) => {
                    ...
                    return (
                      <span
                        key={wing}
                        style={{
                          background: wc.bg, color: wc.text, fontSize: 11, fontWeight: 600,
