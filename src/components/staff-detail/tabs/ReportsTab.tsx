  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <button onClick={handleExportCSV} className="dg-btn dg-btn-secondary">
      ...
      {/* Distribution charts — 2 equal columns */}
      <div className="staff-grid-2">
        ...
        <div style={sectionStyle}>
          ...
          <div style={sectionBodyStyle}>
            {shiftDist.length === 0 ? (
              <div style={{ color: "var(--color-text-faint)", fontSize: "var(--dg-fs-caption)", textAlign: "center", padding: "16px 0" }}>
      ...
      {/* Overtime Summary — 3 equal stat cards */}
      <div style={sectionStyle}>
        ...
        <div style={{ ...sectionBodyStyle, display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
          <OTStat label="Weeks with OT" value={String(otSummary.weeksWithOT)} isActive={otSummary.weeksWithOT > 0} />
          <OTStat label="Total OT Hours" value={`${otSummary.totalOTHours}h`} isActive={otSummary.totalOTHours > 0} />
          <OTStat label="Avg OT/Week" value={`${otSummary.avgOTPerWeek}h`} isActive={otSummary.avgOTPerWeek > 0} />
        </div>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {[...hoursHistory].reverse().map((week) => (
              <tr key={week.weekStart} className="dg-table-row">
...
function OTStat({ label, value, isActive }: { label: string; value: string; isActive: boolean }) {
  return (
    <div style={{ textAlign: "center" }}>
      <div style={{ fontSize: "var(--dg-fs-caption)", fontWeight: 500, color: "var(--color-text-subtle)", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
        <span style={{ width: 6, height: 6, borderRadius: "50%", background: isActive ? "var(--color-danger)" : "var(--color-border)", flexShrink: 0 }} />
        {label}
      </div>
      <div style={{ fontSize: 24, fontWeight: 700, color: isActive ? "var(--color-danger)" : "var(--color-text-primary)", marginTop: 4, letterSpacing: "-0.02em" }}>
        {value}
      </div>
    </div>
  );
...
function DonutChart({
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      <circle cx={center} cy={center} r={radius} fill="none" stroke="var(--color-border-light)" strokeWidth={14} />
      {arcs.map((arc, i) => (
        <circle
          key={i}
          cx={center}
