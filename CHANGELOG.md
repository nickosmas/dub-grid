...
- **Management access popup edits departments only** - the "Edit management access" popup no longer carries role or permission controls, so the same person gets the same popup from the People slide-over and from their profile page. Role changes live on the person's own surface instead: the slide-over's header select and the profile page's Access card. The popup asks for a role only when creating a brand-new invitation, where nothing yet exists to hold one. The permissions launcher moved out of that popup and onto the slide-over itself, which was previously its only route.
- **"Remove from Schedule" and "Remove from Management" buttons removed** - both were one-tap shortcuts for clearing the tags directly beneath them, so deselecting those tags and saving is now the single removal path. What saving would take away is stated once per section by a `SectionNotice` banner (filled, with a solid caution glyph) at the top of its section or, in a popup, directly under the title; field validation stays as plain coloured text under its own input.
