...
| `dg_device`                   | Essential | Random id for this browser, used only to skip the new-sign-in email for a browser the user has signed in from before. The server stores a SHA-256 of it per user (`user_known_devices`, migration 048); a password change forgets every device. `HttpOnly`, read and written server-side only. | 400 days (renewed at each sign-in) | Server Route Handler (`/api/auth/track-session`) |
