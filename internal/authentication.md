...
| Remaining choice  | Enforcement is account-based, not role-based: enrolling a factor is optional and unprompted, with no banner or reminder; people enable it under Profile > Security. It is not a hard block. Gating `/settings` and `/gridmaster` behind AAL2 for privileged roles remains a deliberate future decision so existing admins are not locked out.                                                                                                                                 |
