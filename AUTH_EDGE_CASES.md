...
| A4 | Low | `switch_org` RPC now rejects **suspended** orgs for normal members (was archived-only); gridmaster oversight path unchanged. *Needs `npm run db:reset` to apply in an existing env.* | `migrations/002_functions_triggers.sql` |
...
- **L-4** — onboarding step persists the *clamped* index.
