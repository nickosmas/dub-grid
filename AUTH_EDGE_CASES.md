...
| B1  | Med | Per-**target-email** rate limit (`emailTargetLimiter`, 5/h, keyed by `hashEmail`) added to invite-email and gridmaster password-reset — the per-actor limit alone let one actor email-bomb a single inbox.                                                                           | `lib/rate-limit.ts` + `organizations/invitations/create`, `gridmaster/password-reset` |
